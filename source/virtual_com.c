...
#define USB_CANCELLED_TRANSFER_LENGTH (0xFFFFFFFFU) //nsmoon@230314
...
volatile static uint32_t s_recvSize;
volatile static uint32_t s_sendSize;
volatile static usb_device_composite_struct_t *g_deviceComposite;

#if 1 //nsmoon@230321
USB_DMA_NONINIT_DATA_ALIGN(USB_DATA_ALIGN_SIZE) static uint8_t s_currRecvLine[DATA_BUFF_SIZE];
volatile static uint32_t s_recvLineSize;
volatile static uint32_t s_recvLineFlag; //1:found CR
static void add_line_buff(uint8_t ch);
static int DEBUG_ParseCmd(void);
#endif
...
usb_status_t USB_DeviceCdcVcomCallback(class_handle_t handle, uint32_t event, void *param)
{
    ...
    //usb_echo("event=%d %x %x\r\n", event, acmReqParam, epCbParam);
...
void USB_DeviceCdcVcomTask(void)
{
    ...
    if ((1 == g_deviceComposite->cdcVcom.attach) && (1 == g_deviceComposite->cdcVcom.startTransactions))
    {
        ...
        if ((0 != s_recvSize) && (USB_CANCELLED_TRANSFER_LENGTH != s_recvSize))
        {
            ...
            {
            	uint8_t ch = s_currRecvBuf[i];
            	if (s_sendSize < DATA_BUFF_SIZE - 1) {
                    s_currSendBuf[s_sendSize++] = ch;

            	    add_line_buff(ch); //nsmoon@230321
            	}
            }
            s_recvSize = 0;
        }

        if (DEBUG_ParseCmd()) { //nsmoon@230321
        	if (s_sendSize < DATA_BUFF_SIZE - 1) {
                s_currSendBuf[s_sendSize++] = '\n'; //FIXME
        	}
        }
...
// VCOM Command Parser
///////////////////////////////////////////////
#define TRACE_DPC(...)    //usb_echo(__VA_ARGS__)

static void add_line_buff(uint8_t ch)
{
	//TRACE_DPC("ch=[%x]\r\n", ch);

	if(vcom_mode==VCOM_DAC_MODE)
	{
		Current_adj_mode(ch);
	}
	else
	{
	   ...
	   {
			s_currSendBuf[s_sendSize++] = ' ';
			s_currSendBuf[s_sendSize++] = '\b';
			if(s_recvLineSize > 0)s_recvLineSize--;
	   }

		if (ch =='\r' && s_recvLineSize > 0) {
			s_currRecvLine[s_recvLineSize] = ch;	//'\0';
			s_recvLineFlag = 1;
		}
		else if (ch >= 0x20 && ch <= 0x7e) { //from 'space' to '~'
			if (s_recvLineSize < DATA_BUFF_SIZE - 1) {
				s_currRecvLine[s_recvLineSize++] = ch;
			}
		}
	}
}
...
static int DEBUG_ParseCmd(void)
{
    if ((1 == g_deviceComposite->cdcVcom.attach) && (1 == g_deviceComposite->cdcVcom.startTransactions))
    {
		if (!s_recvLineFlag || s_recvLineSize <= 0) {
			return 0; //not-found
		}
		TRACE_DPC("s_currRecvLine=[%s]\r\n", s_currRecvLine);
		shellTask(&s_currRecvLine[0], s_recvLineSize);
	#if 0 //for test
		if (strcmp((char *)s_currRecvLine, "hello") == 0) {
			strcpy((char *)&s_currRecvLine[5], " world\r\n");
			USB_DeviceCdcAcmSend(g_deviceComposite->cdcVcom.cdcAcmHandle,
								 USB_CDC_VCOM_DIC_BULK_IN_ENDPOINT, &s_currRecvLine[5], 8);
		}
	#endif
		s_recvLineFlag = 0;
		s_recvLineSize = 0;
...
#include <stdarg.h>
#include <stdlib.h>
int DbgVcom_SendData(uint8_t *ch, size_t size)
{
    usb_status_t ret = 0;
    uint32_t len;
    ...
    if ((1 == g_deviceComposite->cdcVcom.attach) && (1 == g_deviceComposite->cdcVcom.startTransactions))
    {
    	if(nr_option == 0) len = size + 2; //cr+lf
    	else len = size;

		if (len > FS_CDC_VCOM_BULK_IN_PACKET_SIZE) {
		    len = FS_CDC_VCOM_BULK_IN_PACKET_SIZE;
		}
		if(nr_option == 0)
		{
			ch[len-1] = '\n';
		...
		ret = USB_DeviceCdcAcmSend(g_deviceComposite->cdcVcom.cdcAcmHandle,
				USB_CDC_VCOM_DIC_BULK_IN_ENDPOINT, ch, len);
    }
    return ret;
}
#endif
#endif //(USB_DEVICE_CONFIG_CDC_ACM > 0)
