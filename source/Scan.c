...
void onofftime_freq_Calc(axis_type_enum axisType, float Ontime, float Offtime)
{
	...
	sum = Ontime+Offtime;
	timergp[PWM_TIMER_1_2].pwmFreqHz = (uint32_t)(1/sum*1000);
	...
	timergp[PWM_TIMER_3].pwmFreqHz =(uint32_t)(1/sum*1000);
	timergp[PWM_TIMER_3].duty =(uint32_t)(t3_on_calc/sum*100);
}
...
void setTestTimerMsec(uint32_t val)
{
	endtestTimer = (uint32_t)(WAIT_TIME_10MS(val));	// 50
	    //TIMER_CNT(TIMER3) = 0;
	startTestTimer = GPT_GetCurrentTimerCount(BOARD_GPT2_PERIPHERAL);
}
...
{
    if ( GPT_GetCurrentTimerCount(BOARD_GPT2_PERIPHERAL) - startTestTimer >= endtestTimer ) return 1;
    else return 0;
}
#endif
