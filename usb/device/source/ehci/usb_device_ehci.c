...
static uint8_t myUsbSpeed; //nsmoon@201211, should be initialized by USB_DeviceEhciFullSpeed() before call USB device init
#endif
#if USB_DEVICE_SOF_ENABLE //nsmoon@211222
static uint32_t sofIntCnt;
static usb_device_ehci_state_struct_t *ehciStateSav;
#endif
...
#if defined(USB_DEVICE_FULL_SPEED_ENABLE) && (USB_DEVICE_FULL_SPEED_ENABLE > 0U)
    if (myUsbSpeed == USB_SPEED_FULL) {
        ehciState->registerBase->PORTSC1 |= USBHS_PORTSC1_PFSC(0x1U); //nsmoon@201108, full-speed
    }
    else {
        ehciState->registerBase->PORTSC1 &= USBHS_PORTSC1_PFSC(0x1U); //nsmoon@201211, high-speed
    }
#endif
                                               ...
                                               usb_device_endpoint_init_struct_t *epInit)
{
    ...
    if ((USB_CONTROL_ENDPOINT == endpoint)) //nsmoon@200608
    {
...
#if (defined(USB_DEVICE_CHARGER_DETECT_ENABLE) && (USB_DEVICE_CHARGER_DETECT_ENABLE > 0U)) && \
    (defined(FSL_FEATURE_SOC_USBHSDCD_COUNT) && (FSL_FEATURE_SOC_USBHSDCD_COUNT > 0U))
void USB_DeviceEhciFullSpeed(uint8_t usb_speed) //nsmoon@201211
{
	myUsbSpeed = usb_speed; //0: high speed
}
#endif
#if USB_DEVICE_SOF_ENABLE //nsmoon@211222
void USB_DeviceEhciGetSofIntCnt(uint32_t *cnt) //nsmoon@201211
{
	(*cnt) = sofIntCnt;
}
#endif
...
#if USB_DEVICE_SOF_ENABLE //nsmoon@211222
void USB_SOF_IntEnable(void)
{
	if (ehciStateSav != NULL) {
        ehciStateSav->registerBase->USBINTR |= USBHS_USBINTR_SRE_MASK;
	}
	else {
        usb_echo("ERROR! ehciStateSav is NULL");
	}
}
#endif
