                                              ...
                                              usb_host_ehci_pipe_t *ehciPipePointer)
{
    ...
    if (ehciPipePointer->uframeInterval == 0) { //FIXME, nsmoon@190108
#ifdef HOST_ECHO
    	usb_echo("ERROR! ehciPipePointer->uframeInterval\r\n");
#endif
    	ehciPipePointer->uframeInterval = 1;
    }
...
#if 0	//def USB_HOST_DELAYED_INT_ENABLE
     ehciInstance->ehciIpBase->USBINTR |= (0x1000037); //nsmoon@211201 move to USB_HostEhciIntEnable()
#endif
...
#if 1 //def USB_HOST_DELAYED_INT_ENABLE	//nsmoon@211201
void USB_HostEhciIntEnable(void *hostHandle)
{
    if (hostHandle == NULL) {
        return;
    }

    usb_host_ehci_instance_t *ehciInstance = (usb_host_ehci_instance_t *)((usb_host_instance_t *)hostHandle)->controllerHandle;
    /* enable interrupt (USB interrupt enable + USB error interrupt enable + port change detect enable + system error
     * enable + interrupt on async advance enable) + general purpos Timer 0 Interrupt enable */
