...
#include "backend.h" //nsmoon@230404
...
#if(DEBUG_VCOM_ENABLE == DEBUG_VCOM_MODE) //nsmoon@230404
extern int DbgVcom_SendData(uint8_t *ch, size_t size);
#endif
...
int DbgConsole_Vprintf(const char *fmt_s, va_list formatStringArg)
{
    ...
    if (NULL != g_serialHandle)
    {
        ...
        result = DbgVcom_SendData((uint8_t *)printBuf, (size_t)logLength);
#else
